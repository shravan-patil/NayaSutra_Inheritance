...
import { Plus, Search, AlertCircle, CheckCircle2, Clock, BarChart3, FileText, TrendingUp } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 relative overflow-hidden">
      ...
      >
        ...
        >
          ...
          >
            ...
            <div>
              ...
              <p className="text-4xl font-bold text-white">{counts.total - counts.pending}</p>
            </div>
          </motion.div>
          ...
          >
            ...
            <div>
              ...
              <p className="text-4xl font-bold text-white">
                {counts.total > 0 ? Math.round(((counts.total - counts.pending) / counts.total) * 100) : 0}%
              </p>
            </div>
          </motion.div>
        </motion.div>
          ...
          >
            ...
            {filteredFirs.length === 0 ? (
              <div className="py-12 text-center">
                ...
                <p className="text-slate-500 text-sm">Create a new FIR to get started</p>
              </div>
            ) : (
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-slate-400 mb-2">{fir?.offense_nature ?? "N/A"}</p>
                          <div className="flex items-center gap-4 text-xs text-slate-500">
                            <span>📅 {dateStr}</span>
                            {fir?.investigation_officer && (
                              <span>👮 {fir.investigation_officer}</span>
                            )}
                          </div>
                        </div>
              ...
              <div className="space-y-3">
                <div>
                  <div className="flex justify-between mb-2">
                    <span className="text-sm text-slate-400">Case Resolution</span>
                  ...
                  <div className="w-full h-2 bg-slate-700 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${counts.total > 0 ? ((counts.total - counts.pending) / counts.total) * 100 : 0}%`,
                      }}
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="w-full h-2 bg-slate-700 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${counts.total > 0 ? (counts.pending / counts.total) * 100 : 0}%`,
                      }}
                    />
                  </div>
                </div>
        ...
        <motion.div
          variants={itemVariants}
          className="mt-12 glass-card p-6 rounded-xl border border-emerald-500/30 bg-emerald-500/5"
        >
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">
                All FIR records and investigation files are encrypted and stored securely with blockchain verification.
                Track evidence, manage cases, and maintain complete audit trails.
              </p>
            </div>
          </div>
        </motion.div>
